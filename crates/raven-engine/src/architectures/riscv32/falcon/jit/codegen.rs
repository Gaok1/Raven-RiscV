//! GeraÃ§Ã£o de cÃ³digo nativo via `dynasm-rs` â€” Fase B.
//!
//! # VisÃ£o geral
//!
//! Este mÃ³dulo compila um [`BasicBlock`] em cÃ³digo x86_64 nativo e retorna um
//! [`CompiledBlock`] pronto para execuÃ§Ã£o. O ponto de entrada Ã© [`compile_block`].
//!
...
//! rdi = cpu        â†’ salvo em rbx (callee-saved)
//! rsi = mem        â†’ salvo em r12 (callee-saved)
//! rdx = console    â†’ salvo em r13 (callee-saved)
//!
//! // Retorno:
...
//!   mov r13, rdx      ; console
//!
//! [por instruÃ§Ã£o nÃ£o-terminadora]:
//!   call jit_fetch32  ; contabiliza I-cache
//!   mov cpu.pc = pc + 4
//!   add cpu.instr_count, 1
//!   <cÃ³digo nativo da instruÃ§Ã£o>
//!
//! [terminador]:
...
//! ```
//!
//! # InstruÃ§Ãµes compiladas nativamente (Fase B)
//!
...
//! # Trampolines
//!
//! `dynasm` nÃ£o pode chamar mÃ©todos `&mut self` de Rust diretamente (ABI mismatch).
//! Cada operaÃ§Ã£o de memÃ³ria com efeito colateral vira um `unsafe extern "C" fn` com
//! ABI System V x86_64:
//!
//! ```text
...
//!
//! Para ALU pura, os registradores sÃ£o lidos/escritos diretamente via `offset_of!`:
//!
//! ```text
...
//! ```
//!
//! Isso evita trampolines de registrador e Ã© o principal ganho de desempenho
//! sobre o interpretador (~2â€“4Ã— esperado para loops ALU dominantes).
...
// ---------------------------------------------------------------------------
// Offsets dos campos de Cpu (calculados em tempo de compilaÃ§Ã£o)
// ---------------------------------------------------------------------------
...
/// Contabiliza fetch de instruÃ§Ã£o no I-cache sem retornar o dado.
/// O JIT jÃ¡ tem a palavra decodificada em compile-time; sÃ³ precisamos
...
/// Escreve 2 bytes na memÃ³ria via D-cache. Retorna 0=ok, -1=fault.
...
/// Escreve 4 bytes na memÃ³ria via D-cache. Retorna 0=ok, -1=fault.
...
unsafe extern "C" fn jit_handle_ecall(
    ...
    match unsafe { handle_syscall(code, &mut *cpu, &mut *mem, &mut *console) } {
        Ok(true) => {
            ...
            // finish the sleep inline. ponytail: blocks this thread â€” JIT is
...
/// NÃ£o retorna dado (a palavra jÃ¡ estÃ¡ decodificada em compile-time).
/// Evita Vec::clone() no hit path â€” principal fonte de overhead por instruÃ§Ã£o.
#[inline(always)]
...
/// Emite a atualizaÃ§Ã£o de `cpu.pc` e incremento de `cpu.instr_count`.
/// Deve ser chamado para cada instruÃ§Ã£o, antes do corpo da instruÃ§Ã£o.
#[inline(always)]
...
/// Emite o cÃ³digo para uma instruÃ§Ã£o R-type ALU.
///
/// LÃª rs1 e rs2 diretamente da memÃ³ria, aplica a operaÃ§Ã£o, escreve em rd.
...
/// Emite o cÃ³digo para uma instruÃ§Ã£o I-type ALU.
///
/// LÃª rs1, aplica a operaÃ§Ã£o com `imm`, escreve em rd.
...
// ---------------------------------------------------------------------------
// `compile_block` â€” ponto de entrada pÃºblico
// ---------------------------------------------------------------------------

/// Compila um [`BasicBlock`] em cÃ³digo nativo x86_64.
///
/// Retorna `None` se o assembler falhar (improvÃ¡vel em condiÃ§Ãµes normais).
/// O bloco retornado pode ser inserido no [`CompiledBlockCache`] e executado
/// repetidamente atÃ© ser invalidado por SMC.
pub fn compile_block(block: &BasicBlock) -> Option<Arc<CompiledBlock>> {
    let mut ops = Assembler::new().ok()?;
    let entry = ops.offset();

    // --- Prologue ---
    // ApÃ³s 5 pushes (rbp + 4 callee-saved) + sub 8 â†’ RSP alinhado a 16 bytes.
    dynasm!(ops
        ; push rbp
        ; mov rbp, rsp
        ; push rbx
    ...
    // --- Corpo: instruÃ§Ãµes nÃ£o-terminadoras ---
    ...
    for (i, &word) in block.words.iter().enumerate() {
        ...
        let instr = match decode(word) {
            Ok(i) => i,
            Err(_) => {
                // InstruÃ§Ã£o invÃ¡lida â€” retorna Fault
                dynasm!(ops
        ...
        if is_last {
            // A Ãºltima instruÃ§Ã£o Ã© o terminador
    ...
    if block.words.is_empty() || !fault_requested {
        // Se o bloco nÃ£o emitiu o jmp =>epilogue ainda (apenas para blocks de 0 palavras
        // ou blocos onde emit_terminator nÃ£o encerrou com jmp), garantir saÃ­da.
        // Na prÃ¡tica, emit_terminator sempre emite `jmp =>epilogue`.
    }
...
// ---------------------------------------------------------------------------
// EmissÃ£o de instruÃ§Ãµes nÃ£o-terminadoras
// ---------------------------------------------------------------------------
...
fn emit_non_terminator(ops: &mut Assembler, instr: &Instruction, pc: u32) {
    match *instr {
        ...
        Instruction::Add { rd, rs1, rs2 } => {
            emit_rtype(ops, rd, rs1, rs2, |o| dynasm!(o ; add eax, ecx))
        }
        Instruction::Sub { rd, rs1, rs2 } => {
            emit_rtype(ops, rd, rs1, rs2, |o| dynasm!(o ; sub eax, ecx))
        }
        Instruction::And { rd, rs1, rs2 } => {
            emit_rtype(ops, rd, rs1, rs2, |o| dynasm!(o ; and eax, ecx))
        }
        Instruction::Or { rd, rs1, rs2 } => {
            emit_rtype(ops, rd, rs1, rs2, |o| dynasm!(o ; or eax, ecx))
        }
        Instruction::Xor { rd, rs1, rs2 } => {
            emit_rtype(ops, rd, rs1, rs2, |o| dynasm!(o ; xor eax, ecx))
        }
        ...
        Instruction::Srl { rd, rs1, rs2 } => emit_rtype(
            ops,
            rd,
            rs1,
            rs2,
            |o| dynasm!(o ; and ecx, BYTE 31 ; shr eax, cl),
        ),
        Instruction::Sra { rd, rs1, rs2 } => emit_rtype(
            ops,
            rd,
            rs1,
            rs2,
            |o| dynasm!(o ; and ecx, BYTE 31 ; sar eax, cl),
        ),
        ...
        Instruction::Div { rd, rs1, rs2 } => {
            // RV32M: div-by-zero â†’ -1; signed overflow (MIN/-1) â†’ MIN
            let rs1_off = (X_BASE + rs1 as usize * 4) as i32;
            let rs2_off = (X_BASE + rs2 as usize * 4) as i32;
            ...
            dynasm!(ops
                ...
                ; test ecx, ecx
                ; jnz =>skip
                ; mov eax, -1i32 as i32    // div by zero â†’ -1
                ; jmp =>done
                ; =>skip
        ...
        Instruction::Divu { rd, rs1, rs2 } => {
            // RV32M: div-by-zero â†’ 0xFFFF_FFFF
            let rs1_off = (X_BASE + rs1 as usize * 4) as i32;
            let rs2_off = (X_BASE + rs2 as usize * 4) as i32;
            let skip = ops.new_dynamic_label();
            let done = ops.new_dynamic_label();
        ...
        Instruction::Rem { rd, rs1, rs2 } => {
            // RV32M: rem-by-zero â†’ dividend; signed overflow â†’ 0
            let rs1_off = (X_BASE + rs1 as usize * 4) as i32;
            let rs2_off = (X_BASE + rs2 as usize * 4) as i32;
            let skip = ops.new_dynamic_label();
            let done = ops.new_dynamic_label();
        ...
        Instruction::Addi { rd, rs1, imm } => {
            emit_itype(ops, rd, rs1, imm, |o| dynasm!(o ; add eax, ecx))
        }
        Instruction::Andi { rd, rs1, imm } => {
            emit_itype(ops, rd, rs1, imm, |o| dynasm!(o ; and eax, ecx))
        }
        Instruction::Ori { rd, rs1, imm } => {
            emit_itype(ops, rd, rs1, imm, |o| dynasm!(o ; or eax, ecx))
        }
        Instruction::Xori { rd, rs1, imm } => {
            emit_itype(ops, rd, rs1, imm, |o| dynasm!(o ; xor eax, ecx))
        }
        Instruction::Slti { rd, rs1, imm } => emit_itype(
            ops,
            rd,
            rs1,
            imm,
            |o| dynasm!(o ; cmp eax, ecx ; setl al ; movzx eax, al),
        ),
        Instruction::Sltiu { rd, rs1, imm } => emit_itype(
            ops,
            rd,
            rs1,
            imm,
            |o| dynasm!(o ; cmp eax, ecx ; setb al ; movzx eax, al),
        ),
        ...
        // Qualquer instruÃ§Ã£o nÃ£o listada acima que aparecer em posiÃ§Ã£o
        // nÃ£o-terminal Ã© um bug no scan_block (deveria ter parado antes).
        // Emitimos uma instruÃ§Ã£o ilegal (ud2) para detectar o problema.
        _ => {
            dynasm!(ops ; ud2);
        }
    }
...
enum LoadWidth {
    B8s,  // Lb  â€” byte com sign-extension
    B8u,  // Lbu â€” byte com zero-extension
    H16s, // Lh  â€” halfword com sign-extension
    H16u, // Lhu â€” halfword com zero-extension
    W32,  // Lw
}
...
fn emit_load(ops: &mut Assembler, rd: u8, rs1: u8, imm: i32, width: LoadWidth) {
    ...
    dynasm!(ops
        ...
        // Oops â€” addr was in edi (low 32 of rdi). Let's use a temp register.
    );
...
fn emit_terminator(
    ...
    match *instr {
        ...
        Instruction::Ecall => {
            ...
            // Call jit_handle_ecall(cpu, mem, console) â†’ eax = exit discriminant.
        ...
        // --- FallThrough (cap de 64 instruÃ§Ãµes) ---
        // end_pc+4 Ã© o inÃ­cio do prÃ³ximo bloco; cpu.pc jÃ¡ foi setado pelo
        // emit_pre_instruction da Ãºltima instruÃ§Ã£o.
        _ => {
            dynasm!(ops
                ; mov eax, DWORD exit::CONTINUE as i32
                ; jmp =>epilogue
