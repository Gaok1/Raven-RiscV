...
//!
//! # Duas implementaÃ§Ãµes condicionais
//!
//! Este arquivo define `CompiledBlockCache` em duas variantes:
//!
//! - **Com `--features jit`:** implementaÃ§Ã£o real com `HashMap<u32, Arc<CompiledBlock>>`.
//!   `CompiledBlock` contÃ©m o buffer de cÃ³digo nativo (`dynasmrt::ExecutableBuffer`),
...
//! NecessÃ¡rio para Self-Modifying Code: quando o programa escreve em uma regiÃ£o
//! marcada como executÃ¡vel via `SYS_RAVEN_MAP_EXEC`, o driver notifica o backend
//! via `ExecutionBackend::invalidate`, que delega para este mÃ©todo.
//!
//! **ImplementaÃ§Ã£o:** `retain(|_, b| b.end_pc < start || b.start_pc >= end)` â€” O(N)
//! sobre os blocos em cache. AceitÃ¡vel porque SMC denso nÃ£o Ã© o caso de uso real.

// ---------------------------------------------------------------------------
// ImplementaÃ§Ã£o real â€” apenas com cargo feature "jit"
// ---------------------------------------------------------------------------
...
mod real {
    ...
    /// Assinatura C da funÃ§Ã£o compilada pelo JIT.
    ///
    ...
    pub mod exit {
        /// ExecuÃ§Ã£o normal â€” dispatcher deve continuar.
        pub const CONTINUE: u32 = 0;
        /// Programa aguarda entrada do console.
        pub const AWAIT_INPUT: u32 = 1;
        /// Programa encerrou (`halt` ou syscall de saÃ­da).
        pub const HALTED: u32 = 2;
        /// InstruÃ§Ã£o invÃ¡lida ou acesso de memÃ³ria com falha.
        pub const FAULT: u32 = 3;
    }

    /// Um basic block compilado para cÃ³digo nativo x86_64.
    pub struct CompiledBlock {
        /// PC da primeira instruÃ§Ã£o do bloco.
        pub start_pc: u32,
        /// PC da Ãºltima instruÃ§Ã£o do bloco (inclusive).
        pub end_pc: u32,
        /// NÃºmero de instruÃ§Ãµes compiladas neste bloco.
        pub instruction_count: u32,
        /// Buffer de memÃ³ria executÃ¡vel (alocado via mmap).
        pub code: ExecutableBuffer,
        /// Offset dentro de `code` do ponto de entrada da funÃ§Ã£o.
        pub entry: AssemblyOffset,
    }

    impl CompiledBlock {
        /// Retorna o ponteiro de funÃ§Ã£o para a execuÃ§Ã£o nativa do bloco.
        ///
        /// # Safety
        /// O buffer deve conter cÃ³digo x86_64 vÃ¡lido com assinatura [`BlockFn`].
    ...
    impl CompiledBlockCache {
        ...
        /// Insere um bloco recÃ©m-compilado no cache.
        ...
        pub fn invalidate_range(&mut self, start: u32, end: u32) {
            self.blocks
                .retain(|_, b| b.end_pc < start || b.start_pc >= end);
        }

        /// NÃºmero de blocos no cache.
...
// ---------------------------------------------------------------------------
// Stub â€” quando a feature "jit" nÃ£o estÃ¡ ativa
// ---------------------------------------------------------------------------

#[cfg(not(feature = "jit"))]
mod stub {
    /// Stub sem armazenamento real. Ver doc do mÃ³dulo para a versÃ£o real.
    pub struct CompiledBlockCache {
        _private: (),
    }
    ...
    impl CompiledBlockCache {
        ...
        /// No-op. Fase B com `--features jit` implementa a invalidaÃ§Ã£o real.
