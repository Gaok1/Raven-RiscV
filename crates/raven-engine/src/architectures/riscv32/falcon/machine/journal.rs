...
//! clock value. Each entry is a `ChangeSet` â€” the minimum needed to undo one
...
/// bookkeeping that matches it â€” a single instruction pops the exec-trace and
/// decrements its run count, whereas an interactive edit or a GO-burst
/// checkpoint touches neither.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum StepbackKind {
    /// One *committed* instruction â€” a sequential single-step, or a pipeline
...
pub(super) struct ChangeSet<S> {
    ...
    /// never drift out of the undo history â€” see [`super::Machine`].
    pub pipe_before: S,
    pub rewind: Rewind,
}
