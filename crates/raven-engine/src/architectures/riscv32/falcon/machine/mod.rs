...
//! lying state. The danger with the old `pub cpu` / `pub mem`
...
//! are the journaling methods below. The single silent path â€”
//! [`Machine::cpu_mut_unjournaled`] / [`Machine::mem_mut_unjournaled`] â€” names
...
//! rather than wrapping the bus â€” this is correct even under a write-back cache,
...
//! captured by a cheap clone (a cheap clone of the cache state). Together they make one step
//! exactly reversible.
//!
//! ## Honest limits
//!
...
use crate::capability::PipelineInspect;
...
///
/// `Machine` is generic over the pipeline and snapshots its reversible state
/// into every change-set. Presentation state is owned separately by the host
/// and therefore cannot enter the undo journal.
///
/// `Snapshot` holds *only* the reversible execution state (stage latches,
/// functional-unit occupancy, fetch PC, hazard counters, â€¦) â€” never the UI's
...
pub trait JournaledPipeline {
    /// The reversible slice of the pipeline state. `Clone` is not required â€”
    ...
    /// Optional observer surface for hosts that render pipeline internals.
...
pub struct Machine<P: JournaledPipeline = NoPipeline> {
    ...
    /// Physical configuration mutation goes through [`Machine::pipeline_mut`] (unjournaled).
    pipeline: P,
...
impl<P: JournaledPipeline> Machine<P> {
    ...
    // â”€â”€ Reads (the ~260 existing sites borrow through these) â”€â”€
    ...
    /// sequential step â€” a background hart may have left it elsewhere.
    ///
    ...
    /// per-byte RAM pre-images the cycle writes â€” exactly like a single
    ...
    /// [`Machine::sync_mmu`] rather than an unjournaled hatch â€” that earlier
    ...
    // â”€â”€ Sanctioned edits (each journaled, each undoable by `stepback`) â”€â”€
    ...
    /// pipeline run, whose writes bypass the per-byte log â€” step-back can only
    ...
    /// Number of change-sets currently retained (â‰¤ the journal capacity).
    pub fn journal_depth(&self) -> usize {
        self.journal.len()
    }

    // â”€â”€ Escape hatches: the name announces "this skips the journal" â”€â”€

    /// Mutable CPU access that does **not** journal. For reset / loader /
    /// multi-core sync only â€” all audited. Does not clear existing history, so
    ...
    /// untracked writes that follow. That is exactly the GO/JIT pattern â€”
    /// [`Machine::checkpoint`] then `mem_mut_unjournaled` for the burst â€” and it
    ...
    /// both disjoint borrows simultaneously â€” the execution `ExecCtx`, the
    /// pipeline tick, and the per-step MMU sync â€” cannot stack two separate
    ...
    /// (single-step interpreter, edits, checkpoint) â€” there the current pipeline
