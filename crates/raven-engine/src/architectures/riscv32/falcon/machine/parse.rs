...
//! cell is **rejected** (an [`EditError`]) â€” never silently truncated. The
...
fn parse_bin(input: &str, width: MemWidth) -> Result<u64, EditError> {
    let digits = input
        ...
        .unwrap_or(input);
    let value = u128::from_str_radix(&without_separators(digits), 2).map_err(|_| {
...
fn parse_hex(input: &str, width: MemWidth) -> Result<u64, EditError> {
    let digits = input
        ...
        .unwrap_or(input);
    let value = u128::from_str_radix(&without_separators(digits), 16).map_err(|_| {
        EditError::ParseFailed {
            input: input.to_string(),
        }
    })?;
...
fn parse_unsigned_dec(input: &str, width: MemWidth) -> Result<u64, EditError> {
    let value: u128 = without_separators(input)
        .parse()
        .map_err(|_| EditError::ParseFailed {
            input: input.to_string(),
        })?;
    fits_unsigned(value, width, false)
}
...
fn parse_signed_dec(input: &str, width: MemWidth) -> Result<u64, EditError> {
    ...
    // Two's-complement into the cell width: e.g. -128 in B1 â†’ 0x80.
...
/// `1_000` parses the same way it reads on screen â€” mirroring Rust literals.
