// falcon/cache/cache.rs â€” Single cache level: CacheLine, CacheSet, Cache
...
// â”€â”€ Internal structures â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
pub struct CacheLine {
    ...
    pub dirty: bool,
    pub data: Vec<u8>,
    pub freq: u64,     // LFU: access frequency counter
...
pub struct CacheSet {
    ...
    pub lru_order: VecDeque<usize>,  // front=MRU, back=LRU (LRU & MRU)
    pub fifo_order: VecDeque<usize>, // front=newest, back=oldest (FIFO)
    rand_state: u32,
    pub clock_hand: usize, // Clock: current sweep position
}
...
impl CacheSet {
    ...
    pub fn find_victim(&mut self, policy: ReplacementPolicy) -> usize {
    ...
    pub fn touch(&mut self, way: usize, policy: ReplacementPolicy) {
        match policy {
    ...
    pub fn install(&mut self, way: usize, tag: u32, data: Vec<u8>, policy: ReplacementPolicy) {
...
impl Cache {
    ...
    /// Allocate a line (read-only path â€” I-cache, no dirty eviction writes).
    pub fn allocate_ro(&mut self, addr: u32, ram: &Ram) -> Result<(), FalconError> {
        if !self.config.is_valid_config() {
            return Ok(());
        }
        let tag = self.config.addr_tag(addr);
    ...
    /// Allocate a line (read-write path â€” D-cache, may write back dirty eviction).
    pub fn allocate_rw(&mut self, addr: u32, ram: &mut Ram) -> Result<(), FalconError> {
        if !self.config.is_valid_config() {
            return Ok(());
        }
        let tag = self.config.addr_tag(addr);
    ...
    /// Write 16-bit halfword via D-cache â€” single tag lookup (correct stats for `sh`).
    ...
    /// Write 32-bit word via D-cache â€” single tag lookup (correct stats for `sw`).
...
// â”€â”€ UI introspection structs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
