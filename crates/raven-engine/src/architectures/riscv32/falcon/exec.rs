...
pub fn apply_mret<B: Bus>(cpu: &mut Cpu, mem: &mut B) {
    ...
    cpu.pc = cpu.mepc;
    // mstatus: MIE â† MPIE; MPIE â† 1; MPP â† U(=0).
    let mpie = (cpu.mstatus >> 7) & 0x1;
    cpu.mstatus = (cpu.mstatus & !((1 << 3) | (1 << 7) | (0x3 << 11))) | (mpie << 3) | (1 << 7);
}

/// Execute `sret`: restore `pc = sepc` and `priv_mode = sstatus.SPP`.
/// SPP is a single bit (8): 1 â†’ S, 0 â†’ U (supervisor cannot return to M).
/// Per the privileged spec we also set SIEâ†SPIE, SPIEâ†1, SPPâ†U(=0).
pub fn apply_sret<B: Bus>(cpu: &mut Cpu, mem: &mut B) {
    ...
    mem.set_priv_mode(new_mode);
    cpu.pc = cpu.sepc;
    // sstatus: SIE â† SPIE; SPIE â† 1; SPP â† U(=0).
    let spie = (cpu.sstatus >> 5) & 0x1;
    cpu.sstatus = (cpu.sstatus & !((1 << 1) | (1 << 5) | (1 << 8))) | (spie << 1) | (1 << 5);
}
...
/// `mepc`, `cause`/`tval` in their CSRs, swaps `mstatus.MPP` â† prior priv,
/// switches to M-mode, and sets `pc = mtvec & ~3`. With `mtvec == 0` we have
/// no handler to dispatch to â€” surface a fatal error so the harness halts.
///
...
pub fn handle_trap<B: Bus>(
    ...
    if cpu.priv_mode != PrivMode::M && cause < 32 && (cpu.medeleg >> cause) & 1 == 1 {
        ...
        // sstatus: SPP â† prior priv (S=1, U=0); SPIE â† SIE; SIE â† 0.
        let spp = if cpu.priv_mode == PrivMode::S { 1 } else { 0 };
        let sie = (cpu.sstatus >> 1) & 1;
        cpu.sstatus = (cpu.sstatus & !((1 << 1) | (1 << 5) | (1 << 8))) | (spp << 8) | (sie << 5);
    ...
    // Always record the trap CSRs â€” even when there is no installed handler,
    ...
    cpu.mstatus = (cpu.mstatus & !((0x3 << 11) | (1 << 7) | (1 << 3))) | (mpp << 11) | (mie << 7);
...
fn step_inner<B: Bus>(
    ...
    match instr {
        ...
        Instruction::Halt => {
            // Permanent single-hart stop â€” same semantics as FALCON_HART_EXIT.
        ...
        Instruction::Ebreak => {
            // Resumable breakpoint â€” pauses this hart; pressing step/run continues.
            cpu.ebreak_hit = true;
            console.push_colored(
        ...
        Instruction::SfenceVma { rs1, .. } => {
            // rs1=0 â†’ full flush; rs1â‰ 0 â†’ flush only the page containing the
...
// em src/architectures/riscv32/falcon/exec.rs (logo abaixo de `step`)
#[allow(dead_code)]
