...
/// `li rd, imm` â€” load any 32-bit immediate into `rd`.
/// For values in [-2048, 2047] emits a single `addi rd, x0, imm`.
/// For larger values emits `lui rd, hi20` + `addi rd, rd, lo12`.
pub fn parse_li(s: &str, consts: &HashMap<String, i64>) -> Result<Vec<Instruction>, String> {
...
pub fn parse_push(s: &str) -> Result<(Instruction, Instruction), String> {
    // "push rs"
    let mut parts = s.split_whitespace();
    parts.next();
    let rest = parts.collect::<Vec<_>>().join(" ");
...
pub fn parse_pop(s: &str) -> Result<(Instruction, Instruction), String> {
    // "pop rd"
    let mut parts = s.split_whitespace();
    parts.next();
    let rest = parts.collect::<Vec<_>>().join(" ");
...
pub fn parse_print(s: &str) -> Result<Vec<Instruction>, String> {
    // "print rd"
    let mut parts = s.split_whitespace();
    parts.next();
    let rest = parts.collect::<Vec<_>>().join(" ");
...
pub fn parse_print_str(s: &str, labels: &HashMap<String, u32>) -> Result<Vec<Instruction>, String> {
    // "print_str label" â€” Linux write(1, buf, strlen(buf))  [syscall 64]
    ...
    Ok(vec![
        ...
        Instruction::Jal { rd: 0, imm: -12 }, // jal x0, -12 â†’ lbu
...
pub fn parse_print_strln(
    s: &str,
    labels: &HashMap<String, u32>,
) -> Result<Vec<Instruction>, String> {
    // "print_str_ln label" â€” same as print_str + write(1, "\n", 1)  [syscall 64]
...
pub fn parse_read(s: &str, labels: &HashMap<String, u32>) -> Result<Vec<Instruction>, String> {
    // "read label" â€” Linux read(0, buf, 256)  [syscall 63]
...
pub fn parse_read_byte(s: &str, labels: &HashMap<String, u32>) -> Result<Vec<Instruction>, String> {
...
pub fn parse_read_half(s: &str, labels: &HashMap<String, u32>) -> Result<Vec<Instruction>, String> {
...
pub fn parse_read_word(s: &str, labels: &HashMap<String, u32>) -> Result<Vec<Instruction>, String> {
...
pub fn parse_random(s: &str) -> Result<Vec<Instruction>, String> {
    // "random rd" â€” getrandom(syscall 278) into temp stack slot, then lw rd, 0(sp)
...
pub fn parse_random_bytes(
    s: &str,
    labels: &HashMap<String, u32>,
) -> Result<Vec<Instruction>, String> {
    // "random_bytes label, n" â€” getrandom(buf=label, len=n, flags=0)
