...
pub fn assemble(text: &str, base_pc: u32) -> Result<Program, AsmError> {
    ...
    let mut label_source_lines = HashMap::<String, usize>::new(); // label â†’ source line (0-based)
    let mut equates = Vec::<EquateDef>::new();
    // fixups for `.word label` â€” resolved after labels map is built
            ...
            Section::Data => {
                if let Some(rest) = line.strip_prefix(".align") {
                    ...
                    for w in rest.split(',') {
                        ...
                        {
                            // label reference â€” emit placeholder, resolve after labels are built
    ...
    // Build addrâ†’labels reverse map and labelâ†’source-line map
...
/// Called during the first pass before equates are resolved; equate names conservatively â†’ 2 words.
...
fn parse_instr(
    ...
    match mnemonic.as_str() {
        ...
        // Pseudo: bez / beqz rs, label  â†’  beq rs, x0, label
        //         bnez      rs, label  â†’  bne rs, x0, label
        "bez" | "beqz" => {
            if ops.len() != 2 {
        ...
        // ---------- Branch pseudos ----------
        // Two-register: bgt/ble/bgtu/bleu rs, rt, label  (swap operands â†’ blt/bge/bltu/bgeu rt, rs)
        "bgt" | "ble" | "bgtu" | "bleu" => {
            if ops.len() != 3 {
        ...
        "blez" => {
            ...
            Ok(Bge { rs1: 0, rs2, imm }) // bge x0, rs  âŸº  rs â‰¤ 0
        }
        "bgtz" => {
            if ops.len() != 2 {
            ...
            Ok(Blt { rs1: 0, rs2, imm }) // blt x0, rs  âŸº  rs > 0
        }
        ...
        // Pseudo: csrw csr, rs  â†’  csrrw x0, csr, rs
        "csrw" => {
            if ops.len() != 2 {
        ...
        // Pseudo: csrr rd, csr  â†’  csrrs rd, csr, x0
        "csrr" => {
            if ops.len() != 2 {
        ...
        // Pseudo: csrs csr, rs  â†’  csrrs x0, csr, rs
        "csrs" => {
            if ops.len() != 2 {
        ...
        // Pseudo: csrc csr, rs  â†’  csrrc x0, csr, rs
        "csrc" => {
            if ops.len() != 2 {
        ...
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ RV32F â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // Comparison (result â†’ integer rd)
        "feq.s" => {
            if ops.len() != 3 {
        ...
        // Pseudos FP
        "fmv.s" => {
            // fmv.s frd, frs â†’ fsgnj.s frd, frs, frs
            if ops.len() != 2 {
        ...
        "fneg.s" => {
            // fneg.s frd, frs â†’ fsgnjn.s frd, frs, frs
            if ops.len() != 2 {
        ...
        "fabs.s" => {
            // fabs.s frd, frs â†’ fsgnjx.s frd, frs, frs
            if ops.len() != 2 {
...
fn parse_csr_name(s: &str) -> Result<u16, String> {
    ...
    // Numeric literal
    let n = parse_imm(s).ok_or_else(|| {
        format!("unknown CSR: '{s}' (use a name like satp/mstatus or a hex number)")
    })?;
    if n < 0 || n > 0xFFF {
