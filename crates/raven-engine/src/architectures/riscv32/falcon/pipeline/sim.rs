...
use super::PipelineTiming as CpiConfig;
use super::{
        ...
        Instruction::Ebreak | Instruction::Halt => {
            ...
            if matches!(instr, Instruction::Halt) {
                ...
                console.push_colored(
                    ...
                    crate::host::console::ConsoleColor::Info,
                );
            } else {
                cpu.ebreak_hit = true;
                ...
                console.push_colored(
                    ...
                    crate::host::console::ConsoleColor::Warning,
                );
            }
            return false;
        }
...
fn pipeline_enter_trap(
    ...
    match crate::falcon::exec::handle_trap(
        cpu, mem, console, entry_pc, trap.cause, trap.tval, trap.vaddr,
    ) {
        Ok(true) => {
        ...
        if let Some(err) = fetch_error {
            match err {
                Ok(trap) => {
                    ...
                    // CPU borrow again.
                    state.pending_fetch_trap = Some((entry_pc, trap.cause, trap.tval, trap.vaddr));
                }
                Err(msg) => {
...
fn fetch_slot(pc: u32, mem: &mut CacheController) -> (Option<PipeSlot>, FetchError) {
    ...
    match result {
        ...
        Err(crate::falcon::errors::FalconError::Trap { cause, tval, vaddr }) => {
            (None, Some(Ok(MemTrap { cause, tval, vaddr })))
        }
