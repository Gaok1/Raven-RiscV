...
impl Falcon {
    ...
    /// Number of harts. Only 1 is supported here; use `raven::cli::run_headless`
    ...
    pub fn run(self) -> Result<RunResult, String> {
        ...
        let src = self
            .asm
            .as_deref()
            .ok_or("no program: call .asm(...) first")?;
        ...
        loop {
            ...
            // ponytail: ignore cpu.sleep_until â€” tests should not wall-clock sleep.
            match outcome {
...
impl RunResult {
    ...
    /// Word at `addr` (cache-coherent â€” sees values still in a writeback cache).
...
#[cfg(test)]
#[path = "../../../../tests/unit/falcon_engine.rs"]
mod tests;
