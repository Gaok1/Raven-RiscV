...
use super::parse::{CellFormat, parse_cell};
...
fn step_then_stepback_is_identity() {
    ...
    assert_eq!(
        fingerprint(m.cpu()),
        cpu0,
        "CPU must round-trip to its start"
    );
    assert_eq!(
...
fn stepback_restores_memory_store() {
    ...
    assert_eq!(
        m.mem().peek32(DATA_ADDR).unwrap(),
        0,
        "RAM reverted to zero"
    );
}
...
fn instr_word_edit_visible_to_fetch() {
    ...
    let original = encode(Instruction::Addi {
        rd: 1,
        rs1: 0,
        imm: 5,
    })
    .unwrap();
    let replacement = encode(Instruction::Addi {
        rd: 1,
        rs1: 0,
        imm: 9,
    })
...
fn stepback_reports_what_it_undid() {
    ...
    m.write_reg(RegTarget::X(RegId::new(5).unwrap()), 0xABCD)
        .unwrap(); // an edit
...
fn journal_ring_bounded() {
    ...
    assert_eq!(
        m.journal_depth(),
        1024,
        "oldest entries evicted at capacity"
    );
}
...
fn parse_cell_str_packs_little_endian() {
    assert_eq!(
        parse_cell("AB", MemWidth::B2, CellFormat::Str, false),
        Ok(0x4241)
    );
    assert!(matches!(
