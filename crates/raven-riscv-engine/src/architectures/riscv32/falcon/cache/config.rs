// falcon/cache/config.rs â€” CacheConfig struct and address decomposition helpers
...
impl CacheConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.line_size < 4 {
            return Err(format!("Line size must be â‰¥ 4 B (got {})", self.line_size));
        }
        if self.associativity == 0 {
            return Err("Associativity must be â‰¥ 1".to_string());
        }
        ...
        let bytes_per_set = match self.line_size.checked_mul(self.associativity) {
            Some(v) => v,
            None => return Err("assoc Ã— line_size overflows usize".to_string()),
        };
        if bytes_per_set > self.size {
            return Err(format!(
                "assoc({}) Ã— line({}) = {} B > size({} B): need â‰¥ 1 set",
                self.associativity, self.line_size, bytes_per_set, self.size
            ));
        }
        if self.size % bytes_per_set != 0 {
            return Err(format!(
                "size({}) not divisible by assoc({}) Ã— line({})",
