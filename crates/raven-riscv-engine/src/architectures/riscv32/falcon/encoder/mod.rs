// src/architectures/riscv32/falcon/encoder/mod.rs
...
fn r4(rs3: u32, rs2: u32, rs1: u32, rd: u32, opc: u32) -> u32 {
    ...
    // fmt bits [26:25]=0b00 (single precision), rm bits [14:12]=0b000 (RNE) â€” both zero
}
#[inline]
...
pub fn encode(inst: Instruction) -> Result<u32, &'static str> {
    ...
    Ok(match inst {
        ...
        Ebreak => 0x0010_0073, // SYSTEM/EBREAK â€” resumable breakpoint
        Halt => 0x0020_0073,   // Raven HALT  â€” permanent single-hart stop
        Fence => 0x0FF0_000F,  // MISC-MEM/FENCE with pred=iorw, succ=iorw
        FenceI => 0x0000_100F,

        // Zicsr â€” funct3 1..3 = reg form; rs1 holds the source register.
        ...
        // RV32F â€” OP-FP (funct7 encodes operation, funct3=rm=0 for RNE)
        ...
        // RV32F â€” R4-type (fused multiply-add)
