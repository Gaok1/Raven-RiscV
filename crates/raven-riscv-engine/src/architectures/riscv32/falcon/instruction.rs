...
pub enum Instruction {
    ...
    // Sv32 TLB invalidate. rs1=0 â†’ full flush; rs2 (ASID) ignored in Phase 2.
    SfenceVma {
        rs1: u8,
        rs2: u8,
    },

    // Memory ordering (RV32I base â€” executed as nop in single-core simulator)
    Fence,
    FenceI,
    ...
    // RV32F â€” floating-point extension
    ...
    FcvtWS {
        ...
        rs1: u8,
        rm: u8,
    }, // f32 â†’ i32 (signed);  rm = rounding mode (0=rne,1=rtz,2=rdn,3=rup,4=rmm,7=dyn)
    FcvtWuS {
        rd: u8,
        rs1: u8,
        rm: u8,
    }, // f32 â†’ u32 (unsigned); rm = rounding mode
    FcvtSW {
        rd: u8,
        rs1: u8,
    }, // i32 â†’ f32
    ...
    FmvXW {
        rd: u8,
        rs1: u8,
    }, // float-bits â†’ int reg
    FmvWX {
        rd: u8,
        rs1: u8,
    }, // int reg â†’ float-bits
