...
// Falcon graphics extensions (screen device â€” see `crate::ui::screen`)
...
fn should_trace_syscall(code: u32) -> bool {
    !matches!(
        ...
        SYS_READ
            ...
            | FALCON_READ_FLOAT
            | FALCON_PRINT_FLOAT
            // Graphics calls run once per pixel/frame â€” tracing them floods
            // the console. `screen_init` (rare, interesting) stays traced.
            | GFX_SCREEN_CLEAR
            | GFX_SET_PIXEL
            | GFX_FILL_RECT
            | GFX_PRESENT
...
pub fn handle_syscall_with_cycle_override<B: Bus>(
    ...
    match code {
        ...
        SYS_BRK => {
            // brk(0) â†’ query current break; brk(addr) â†’ extend break to addr.
        ...
        SYS_MUNMAP => {
            // munmap: nop â€” Raven has no real virtual memory management.
            cpu.write(10, 0);
            Ok(true)
        }
        ...
        // --- Falcon graphics extensions (screen device) ---
...
// â”€â”€ Graphics syscalls (2000+) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/// `screen_sleep_ms(a0=ms)` â€” parks this hart on its `ecall` without blocking
/// the host. First call arms `cpu.sleep_until` (and already writes `a0=0`, so
/// the JIT path â€” which never re-executes â€” is also correct); the interpreter
...
fn linux_writev<B: Bus>(
    ...
    // struct iovec { void *base; size_t len; } â€” both u32 on RV32
    let fd = cpu.read(10);
...
fn linux_clock_gettime<B: Bus>(cpu: &mut Cpu, mem: &mut B) -> Result<bool, FalconError> {
    ...
    let tp = cpu.read(11);
    // Approximate: 1 instruction â‰ˆ 10 ns (100 MHz equivalent)
