...
/// Returns map from 0-based line number â†’ comment text.
pub fn extract_block_comments(text: &str) -> HashMap<usize, String> {
    text.lines()
        .enumerate()
        .filter_map(|(i, l)| {
            let t = l.trim();
...
pub fn extract_visible_comments(text: &str) -> HashMap<usize, String> {
    text.lines()
        .enumerate()
        .filter_map(|(i, l)| {
            let pos = l.find("#!")?;
...
pub fn parse_reg(s: &str) -> Option<u8> {
...
pub fn parse_char_lit(s: &str) -> Result<i32, String> {
    let inner = s
        .strip_prefix('\'')
    ...
    if ch.is_ascii() {
        ...
        Err(format!(
            "character literal {s} is not ASCII (only ASCII 0â€“127 is supported)"
        ))
    }
}
...
pub fn parse_imm(s: &str) -> Option<i32> {
    let s = s.trim();
    if s.starts_with('\'') {
        parse_char_lit(s).ok()
    } else {
...
pub fn parse_imm64(s: &str) -> Option<i64> {
    let s = s.trim();
    if s.starts_with('\'') {
...
pub fn parse_str_lit(s: &str) -> Option<String> {
    let s = s.trim();
...
pub fn parse_shamt(s: &str) -> Result<u8, String> {
...
pub fn check_signed(imm: i32, bits: u32, ctx: &str) -> Result<i32, String> {
...
pub fn check_u_imm(imm: i32, ctx: &str) -> Result<i32, String> {
...
pub fn parse_memop(op: &str) -> Result<(i32, u8), String> {
    // "imm(rs1)"
    let (imm_s, rest) = op
        .split_once('(')
...
pub fn load_like(ops: &[String]) -> Result<(u8, i32, u8), String> {
    if ops.len() != 2 {
...
pub fn store_like(ops: &[String]) -> Result<(u8, i32, u8), String> {
    if ops.len() != 2 {
...
/// Parse a float register name: f0â€“f31, ft0â€“ft11, fs0â€“fs11, fa0â€“fa7
pub fn parse_freg(s: &str) -> Option<u8> {
    let s = s.trim().to_lowercase();
    // f0..f31
...
pub fn fp_load_like(ops: &[String]) -> Result<(u8, i32, u8), String> {
    if ops.len() != 2 {
...
pub fn fp_store_like(ops: &[String]) -> Result<(u8, i32, u8), String> {
    if ops.len() != 2 {
...
/// Parse a floating-point rounding mode name â†’ rm bits (RISC-V ISA encoding).
/// rne=0, rtz=1, rdn=2, rup=3, rmm=4, dyn=7
pub fn parse_rm(s: &str) -> Option<u8> {
