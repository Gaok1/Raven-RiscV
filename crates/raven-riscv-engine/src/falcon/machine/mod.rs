...
/// change-set, and restores it on undo â€” without `falcon` depending on `ui`.
///
...
impl<P: JournaledPipeline> Machine<P> {
    ...
    /// Mutable pipeline access for **UI/config** changes only â€” hover flags,
    ...
    /// Number of change-sets currently retained (â‰¤ [`JOURNAL_CAPACITY`]).
