...
Rótulos (`label:`) podem ser definidos em qualquer segmento. Para carregar o endereço de um rótulo, use a pseudoinstrução `la rd, label`, que emite um par `lui`/`addi`.
...
- `printString label|rd` → define `a7=2`, imprime string no rótulo/endereço
- `read` → define `a7=3`, lê entrada para a memória apontada por `a0`
...
| 2 | `printString label\|rd` | Imprime a string terminada em NUL no `label` ou endereço em `rd` (`a0=addr`). |
| 3 | `read` | Lê uma linha para a memória em `a0` e adiciona um byte NUL. |
...
- Tipo R (opcode `0x33`): operações registrador–registrador. `rd = OP(rs1, rs2)`. Exemplos: `ADD, SUB, AND, OR, XOR, SLL, SRL, SRA, SLT, SLTU`. Extensão M suportada: `MUL*`, `DIV*`, `REM*`.
- Tipo I (opcode `0x13`): ALU registrador–imediato. `rd = OP(rs1, imm12)`. Imediatos são sign-extended. Shifts (`SLLI/SRLI/SRAI`) usam `shamt` de 5 bits; `SRAI` tem `funct7=0x20`.
- Loads (opcode `0x03`): `LB/LH/LW/LBU/LHU` leem de `rs1 + imm` e escrevem em `rd`. Loads de byte/meia palavra fazem extensão de sinal ou zero.
- Stores (opcode `0x23`): `SB/SH/SW` escrevem os 8/16/32 bits menos significativos de `rs2` em `rs1 + imm`.
- Branches (opcode `0x63`): desvios condicionais relativos ao PC com deslocamento de 13 bits (em bytes): `BEQ/BNE/BLT/BGE/BLTU/BGEU`. O assembler calcula o deslocamento a partir de rótulos.
- U-type (`LUI/AUIPC`): `LUI` carrega os bits [31:12] em `rd` (deslocado 12); `AUIPC` soma o imediato ao `pc` atual.
- Jumps (`JAL/JALR`): `JAL` escreve `pc+4` em `rd` e salta para `pc + imm21`; `JALR` escreve `pc+4` em `rd` e salta para `(rs1 + imm12) & !1`.

Veja [`docs/format.pt-BR.md`](format.pt-BR.md) para layouts de bits (campos, opcodes e imediatos) e mais exemplos.
...
A aba Run possui um console inferior onde as syscalls `print`, `printString` e `read` fazem E/S. `print rd` mostra o valor decimal de um registrador, `printString label|rd` imprime uma string terminada em NUL e `read` armazena uma linha no endereço em `a0`. Role com `Ctrl+Up/Down` para revisar linhas anteriores.
