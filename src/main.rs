...
fn main() -> io::Result<()> {
    ...
    let app = match ui::App::new_with_architecture(ram_override, jit_override, &architecture) {
        Ok(app) => app,
        Err(error) => {
            ratatui::restore();
    ...
    #[cfg(unix)]
    let res = ui::run(&mut terminal, app, quit_flag);
    #[cfg(not(unix))]
...
fn cmd_build(args: &[String]) -> Result<(), String> {
    validate_value_flags(args, &["--out", "--arch"])?;
