...
/// The single CPI table: every instruction maps to a class, and a class maps to
/// cycles. The sequential and pipeline views pass a different `s` (stage
/// overhead) but must otherwise agree, so the match lives here exactly once —
/// a second copy was drifting the two displays apart.
fn classify_cpi_impl(word: u32, cpu: &crate::falcon::Cpu, cpi: &CpiConfig, s: u64) -> u64 {
...
pub(super) fn classify_cpi_cycles(word: u32, cpu: &crate::falcon::Cpu, cpi: &CpiConfig) -> u64 {
    classify_cpi_impl(word, cpu, cpi, cpi.stage_overhead)
}
