...
//!
//! ponytail: the framebuffer is not journaled — step-back rewinds CPU/memory
...
pub struct Screen {
    pub width: u32,
    pub height: u32,
    /// Back buffer — every draw syscall writes here.
    buf: Vec<u32>,
    /// Front buffer — last `screen_present`; this is what gets displayed.
    pub front: Vec<u32>,
...
// ── OS window (minifb), gated exactly like `rfd` in Cargo.toml ──────────────
...
mod window {
    ...
    pub fn spawn(width: u32, height: u32) -> Result<WindowBridge, String> {
        if cfg!(target_os = "macos") {
            ...
            return Err(
                "OS window is not supported on macOS — showing the Run tab Screen view instead"
                    .into(),
            );
        }
...
mod window {
    ...
    pub fn spawn(_width: u32, _height: u32) -> Result<WindowBridge, String> {
        Err("OS window is not available in this build — showing the TUI Screen view".into())
    }
}
