...
fn run_layout(term: Rect, console_height: u16) -> RunLayout {
    ...
    RunLayout {
        controls: Rect { x: c.x, y: c.y,                             width: c.width, height: controls_h },
        main:     Rect { x: c.x, y: c.y + controls_h,                width: c.width, height: main_h },
        console:  Rect { x: c.x, y: c.y + controls_h + main_h,       width: c.width, height: console_h },
    }
}
...
fn sidebar_rect(term: Rect, app: &App) -> Rect {
    ...
    Rect { x: rl.main.x, y: rl.main.y, width: sidebar_w, height: rl.main.height }
}
...
fn imem_rect(term: Rect, app: &App) -> Rect {
    ...
    Rect { x: rl.main.x + sidebar_w, y: rl.main.y, width: imem_w, height: rl.main.height }
}
...
fn trace_rect(term: Rect, app: &App) -> Rect {
    ...
    Rect { x: im.x, y: im.y + imem_h, width: im.width, height: trace_h }
}
