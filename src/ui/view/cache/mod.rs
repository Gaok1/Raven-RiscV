...
fn render_cache_exec_controls(f: &mut Frame, area: Rect, app: &App) {
    let hint = if matches!(app.cache.subtab, crate::ui::app::CacheSubtab::Stats) {
        "   r=reset  f=speed  p=pause  s=capture  ↑↓=history  D=del"
    } else {
        "   r=reset  f=speed  p=pause  s=step"
    };
    crate::ui::view::components::render_exec_controls(
        f,
        area,
        app,
        &app.cache.exec_speed_btn,
        &app.cache.exec_state_btn,
        &app.cache.exec_reset_btn,
        hint,
    );
}
