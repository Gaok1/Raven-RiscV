...
fn fcache_ref_lines_en() -> Vec<Line<'static>> {
    vec![
        ...
        kv(
            "tlb.entry_count",
            "integer, power of 2, >= tlb.associativity (default 32)",
        ),
        kv("tlb.associativity", "integer >= 1 (default 4)"),
        kv(
            "tlb.replacement",
            "lru fifo random lfu clock mru   (lowercase!)",
        ),
...
fn fcache_ref_lines_ptbr() -> Vec<Line<'static>> {
    vec![
        ...
        kv(
            "tlb.entry_count",
            "inteiro, potência de 2, >= tlb.associativity (padrão 32)",
        ),
        kv("tlb.associativity", "inteiro >= 1 (padrão 4)"),
        kv(
            "tlb.replacement",
            "lru fifo random lfu clock mru   (minúsculas!)",
        ),
        kv("tlb.hit_latency", "inteiro >= 1 ciclos (padrão 1)"),
        kv("tlb.miss_penalty", "inteiro >= 0 ciclos (padrão 20)"),
        note(
            "A política da TLB é em minúsculas, diferente da chave .replacement do cache (PascalCase).",
        ),
        blank(),
