...
fn fcache_ref_lines_en() -> Vec<Line<'static>> {
    vec![
        ...
        h2("Unified TLB Keys"),
        blank(),
        kv("tlb.entry_count", "integer, power of 2, >= tlb.associativity (default 32)"),
        kv("tlb.associativity", "integer >= 1 (default 4)"),
        kv("tlb.replacement", "lru fifo random lfu clock mru   (lowercase!)"),
        kv("tlb.hit_latency", "integer >= 1 cycles (default 1)"),
        kv("tlb.miss_penalty", "integer >= 0 cycles (default 20)"),
        note("TLB replacement is lowercase, unlike the cache .replacement key (PascalCase)."),
        blank(),
        note("The VM mode lives in the .rcfg, not here: vm_mode = off|sv32|custom|manual,"),
        note("plus vm_offset_bits and vm_level_bits (comma list) for Custom mode."),
        blank(),
        ...
        blank(),
        mono("  # --- Unified TLB ---"),
        mono("  tlb.entry_count=32"),
        mono("  tlb.associativity=4"),
        mono("  tlb.replacement=lru"),
        mono("  tlb.hit_latency=1"),
        mono("  tlb.miss_penalty=20"),
        blank(),
    ]
}
...
fn fcache_ref_lines_ptbr() -> Vec<Line<'static>> {
    vec![
        ...
        blank(),
        h2("Chaves da TLB Unificada"),
        blank(),
        kv("tlb.entry_count", "inteiro, potência de 2, >= tlb.associativity (padrão 32)"),
        kv("tlb.associativity", "inteiro >= 1 (padrão 4)"),
        kv("tlb.replacement", "lru fifo random lfu clock mru   (minúsculas!)"),
        kv("tlb.hit_latency", "inteiro >= 1 ciclos (padrão 1)"),
        kv("tlb.miss_penalty", "inteiro >= 0 ciclos (padrão 20)"),
        note("A política da TLB é em minúsculas, diferente da chave .replacement do cache (PascalCase)."),
        blank(),
        note("O modo de VM vive no .rcfg, não aqui: vm_mode = off|sv32|custom|manual,"),
        note("mais vm_offset_bits e vm_level_bits (lista por vírgula) no modo Custom."),
        blank(),
