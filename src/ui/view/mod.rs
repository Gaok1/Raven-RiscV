...
fn help_pages(tab: Tab) -> Vec<Vec<HelpEntry>> {
    match tab {
        ...
        Tab::Tlb => vec![vec![
            ("[Tab]", "cycle subviews: Stats → Settings → Entries → vm"),
            ("Stats", "TLB hits, misses, evictions, page faults, hit-rate chart"),
            ("Settings", "edit entries / associativity / replacement / latencies"),
            ("Entries", "table of installed translations (VPN→PPN, perms, A/D, asid)"),
            ("vm", "live satp / privilege / VM-active banner"),
            ("[f]", "flush TLB"),
            ("[Esc]", "cancel field edit"),
            (
                "Translation active?",
                "needs vm_enabled=on AND satp=Sv32 AND priv∈{S,U}; otherwise identity",
            ),
            (
                "Trap delegation",
                "S-mode delegation (sret / medeleg / sstatus.SPP/SPIE) not yet implemented; all traps land in M-mode",
            ),
        ]],
